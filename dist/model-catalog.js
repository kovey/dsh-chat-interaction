...
export const ALLOWED_REASONING_EFFORTS = ['off', 'low', 'high', 'max'];
let cache = null;
/**
 ...
 */
export function parseSettingsCatalog(raw, source = 'settings.yaml') {
 ...
 */
export function sanitizeModelOverride(catalog, input, opts = {}) {
    const applied = { ...input };
    const dropped = [];
    if (!catalog)
        return { applied, dropped };
    if (applied.model && catalog.models.length > 0 && !catalog.models.includes(applied.model)) {
        dropped.push(`model "${applied.model}" is not in ${catalog.source} (available: ${catalog.models.join(', ')})`);
    ...
    if (applied.reasoningEffort && !allowed.includes(applied.reasoningEffort)) {
        dropped.push(`reasoningEffort "${applied.reasoningEffort}" is not supported (allowed: ${allowed.join(', ')})`);
        applied.reasoningEffort = undefined;
    }
    return { applied, dropped };
}
/** Log a message once per distinct text (keeps the log readable). */
export function warnOnce(message, log = defaultLog) {
...
export function reportDropped(dropped, log = defaultLog) {
 ...
 */
export function isModelAvailable(catalog, model) {
