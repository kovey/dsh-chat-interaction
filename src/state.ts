...
export function projStateDir(cwd = process.cwd()): string {
    try {
...
export function expandHome(p: string | null | undefined): string {
    ...
    if (p === '~/.dsh') return dshHome()
    if (p.startsWith('~/.dsh/')) return path.join(dshHome(), p.slice('~/.dsh/'.length))
 ...
 */
export function statePaths(channel: string, cwd = process.cwd(), cfg: { cardStoreDir?: string; mediaRetentionDays?: number } = {}) {
...
export function writeActiveChat(paths: ReturnType<typeof statePaths>, chatId: string, cwd = process.cwd()): void {
    if (!chatId) return
    try { fs.writeFileSync(paths.activeChat, chatId + '\n') } catch { /* best effort */ }
    // Only non-repo contexts may write the GLOBAL file (multi-project isolation).
    if (!inGitRepo(cwd)) {
        try { fs.writeFileSync(paths.globalActiveChat, chatId + '\n') } catch { /* best effort */ }
    }
}
...
export function pruneOldFiles(dir: string, maxAgeMs: number): void {
    try {
        const now = Date.now()
        for (const f of fs.readdirSync(dir)) {
            const fp = path.join(dir, f)
            try {
                if (fs.statSync(fp).isFile() && now - fs.statSync(fp).mtimeMs > maxAgeMs) fs.unlinkSync(fp)
