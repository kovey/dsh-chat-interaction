...
const CORE_LINES = (d: ChannelDescriptor) => [
...
export const TASK_POLICY_LINES = (d: ChannelDescriptor) => [
...
export const SCORING_PROMPT_LINES = (d: ChannelDescriptor) => [
...
export function buildPromptSection(desc: ChannelDescriptor, opts: PromptSectionOptions = {}): string {
    const lines = [...CORE_LINES(desc)]
    if (opts.taskPolicy !== false) lines.push(...TASK_POLICY_LINES(desc))
    if (opts.scoring) lines.push(...SCORING_PROMPT_LINES(desc))
 ...
 */
export function formatInbound(desc: ChannelDescriptor, msg: InboundMessage, note?: string, score?: ScoreResult): string {
    ...
    if (msg.docLinks && msg.docLinks.length) lines.push(`文档链接: ${msg.docLinks.map((d) => d.url).join(' ')}`)
    if (msg.imagePaths && msg.imagePaths.length) lines.push(`图片文件: ${msg.imagePaths.join(' ')}`)
    if (msg.imageErrors && msg.imageErrors.length) lines.push(`图片下载失败: ${msg.imageErrors.join('; ')}`)
    if (msg.isBotMentioned) lines.push('注意: 群聊中 @提及了本机器人')
    lines.push(`内容: ${msg.text}`)
    if (note) lines.push(`插件上下文: ${note}`)
    if (score) {
        lines.push(`消息评分: ${score.score.toFixed(2)} (${score.level}) → 执行模型: ${score.model || '会话默认'} [${score.source}]`)
        if (score.reasoning) lines.push(`评分依据: ${score.reasoning}`)
    }
    lines.push('')
    lines.push('处理要求:')
