...
// ---------------------------------------------------------------------------
// structural types for the official SDK (loaded lazily)
// ---------------------------------------------------------------------------
 ...
 */
export function resolveWeComBotCreds(cwd: string, cfg: WeComBotChannelConfig): WeComBotCreds {
    const candidates = [
...
export class WeComBotChannel extends BaseChannel {
    ...
    private readonly cfg: WeComBotChannelConfig
    private client: AiBotClientLike | null = null
    private sdk: AiBotSdkLike | null = null
    private disposed = false
    private credsCache: WeComBotCreds | null = null
    ...
    private creds(): WeComBotCreds {
        if (!this.credsCache) this.credsCache = resolveWeComBotCreds(process.cwd(), this.cfg)
    ...
    private async loadSdk(): Promise<AiBotSdkLike> {
        ...
        const mod = (await import('@wecom/aibot-node-sdk')) as unknown as {
            default?: AiBotSdkLike
            WSClient?: AiBotSdkLike['WSClient']
            generateReqId?: AiBotSdkLike['generateReqId']
        }
        this.sdk = {
    ...
    override async connect(): Promise<ListenerStatus> {
        ...
        if (this.client) return { ok: true, connected: true, message: 'already connected' }
        ...
        try {
            ...
            const client = new sdk.WSClient({
                ...
                logger: {
                    debug: (...a: unknown[]) => log('debug', '[aibot]', ...a),
                    info: (...a: unknown[]) => log('info', '[aibot]', ...a),
                    warn: (...a: unknown[]) => log('warn', '[aibot]', ...a),
                    error: (...a: unknown[]) => log('error', '[aibot]', ...a),
                },
            })
    ...
    override async disconnect(): Promise<ListenerStatus> {
        ...
        try {
            ...
            else log('warn', 'aibot client has no disconnect()/close()')
        } catch (e) {
            log('warn', 'aibot disconnect failed:', (e as Error).message)
        }
    ...
    private registerHandlers(client: AiBotClientLike): void {
        const on = (event: string, handler: (frame: AiBotFrameLike) => unknown) => {
            try { client.on(event, handler) } catch (e) {
                log('warn', `aibot client.on(${event}) failed:`, (e as Error).message)
            }
        }
        on('authenticated', () => {
            this.setConnected(true)
        ...
        on('message', (frame) => {
            void this.normalizeMessage(frame).then((msg) => { if (msg) this.emit(msg) })
        })
        ...
        on('event.enter_chat', () => { /* welcome flow is a deployment concern */ })
        on('event.feedback_event', () => { /* telemetry, not a turn */ })
    }

    /** chatid is present for group chats; single chats address the userid. */
    private chatIdOf(body: Record<string, any>): string {
        const chatType = String(body.chattype || '')
        if (chatType === 'group') return String(body.chatid || '')
        return String(body.from?.userid || body.chatid || '')
    }

    private async normalizeMessage(frame: AiBotFrameLike): Promise<InboundMessage | null> {
        ...
        if (msgType === 'image' || (msgType === 'mixed' && body.image)) {
            ...
            if (url) {
                ...
                else imageErrors.push(`${url.slice(-12)}: 图片下载/解密失败`)
            }
        }
    ...
    private textOf(msgType: string, body: Record<string, any>): string {
        switch (msgType) {
            case 'text':
                return String(body.text?.content || '')
            case 'mixed': {
                const parts: string[] = []
                if (body.text?.content) parts.push(String(body.text.content))
                if (body.image) parts.push('[图片]')
    ...
    private normalizeCardEvent(frame: AiBotFrameLike): InboundMessage | null {
        const body = (frame.body || {}) as Record<string, any>
        const chatId = this.chatIdOf(body)
    ...
    private async downloadImage(url: string, aesKey?: string): Promise<string | null> {
        const client = this.client
        if (!client || typeof client.downloadFile !== 'function') return null
        ...
        try {
            ...
            log('warn', 'wecom smart-bot image download failed:', (e as Error).message)
            return null
        }
    }
    ...
    private async requireClient(): Promise<AiBotClientLike> {
    ...
    private async send(chatId: string, body: Record<string, unknown>): Promise<SendResult> {
        try {
...
/** Convenience factory. */
export function createWeComBotChannel(cfg: WeComBotChannelConfig = {}): WeComBotChannel {
