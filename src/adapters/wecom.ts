...
export interface WeComCryptoLike {
    ...
    decrypt?(aesKey: string, encrypt: string): { message: string; id?: string } | string
}
...
export function wecomDecrypt(aesKey: string, encrypt: string): { message: string; id: string } {
 ...
 */
export function resolveWeComCreds(cwd: string, cfg: WeComChannelConfig): WeComCreds {
    const candidates = [
    ...
    for (const f of candidates) {
        ...
        try {
            ...
            for (const [k, v] of Object.entries(j)) {
                if (typeof v === 'string' && v) fromFile[k] = v
                else if (typeof v === 'number' && Number.isFinite(v)) fromFile[k] = String(v)
            }
            break // first existing credential file wins
        } catch { /* try next */ }
    }
    const pick = (cfgVal: unknown, fileKeys: string[], envKeys: string[]): string => {
        if (cfgVal !== undefined && cfgVal !== null && String(cfgVal)) return String(cfgVal)
...
export class WeComChannel extends BaseChannel {
    ...
    private readonly cfg: Required<Pick<WeComChannelConfig, 'taskStoreTtlMs' | 'mediaRetentionDays' | 'timeoutMs'>>
        & WeComChannelConfig
    ...
    private readonly knownChatTypes = new Map<string, 'single' | 'group'>()
    ...
    private credsCache: WeComCreds | null = null
    /**
     * Effective credentials (config → credsFile → project/home wecom-app.json
     * → env WECOM_*), resolved lazily once per adapter instance.
     */
    private creds(): WeComCreds {
        if (!this.credsCache) this.credsCache = resolveWeComCreds(process.cwd(), this.cfg)
        return this.credsCache
    }

    private taskStoreDir(): string {
    ...
    async getAccessToken(): Promise<string> {
        ...
        if (!creds.corpId || !creds.corpSecret) {
            throw new Error(
                'wecom: missing corpId/corpSecret (set them in config, <project>/.dsh/wecom-app.json, ' +
        ...
        this.tokenPromise = (async () => {
            ...
            if (j.errcode !== 0) throw new Error(`wecom gettoken failed: ${j.errcode} ${j.errmsg}`)
    ...
    private async apiJson(pathname: string, body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    private recipientOf(chatId: string): { chatid?: string; touser?: string } {
    ...
    override async sendText(chatId: string, text: string): Promise<SendResult> {
        ...
        while (Buffer.byteLength(rest, 'utf8') > 2048) {
            ...
            while (cut > 0 && Buffer.byteLength(rest.slice(0, cut), 'utf8') > 2048) cut -= 1
        ...
        try {
            for (const chunk of chunks) {
                ...
                if (j.errcode !== 0) return { ok: false, error: `wecom send failed: ${j.errcode} ${j.errmsg}` }
    ...
    override async sendRichText(chatId: string, title: string, body: string): Promise<SendResult> {
        ...
        try {
            ...
            if (j.errcode === 0) return { ok: true, messageId: String(j.msgid || '') }
    ...
    override async sendCard(chatId: string, card: CardSpec): Promise<SendResult> {
        const buttons = (card.buttons || []).map((b, i): ButtonSpec => {
            if (typeof b === 'string') {
                const parts = b.split('|')
                return { value: parts[0] || '', label: parts[1] || parts[0] || '', type: (parts[2] || 'default') as ButtonSpec['type'] }
            }
    ...
    private stashTask(taskId: string, chatId: string, card: CardSpec): void {
        try {
            ...
            log('error', 'task store failed:', (e as Error).message)
        }
    }

    private loadTask(taskId: string): { chatId: string; buttons: ButtonSpec[] } | null {
        try {
            const j = JSON.parse(fs.readFileSync(path.join(this.taskStoreDir(), taskId + '.json'), 'utf8')) as {
                chatId?: string
                buttons?: ButtonSpec[]
            }
            if (j.chatId) return { chatId: j.chatId, buttons: j.buttons || [] }
        } catch { /* not found */ }
        return null
    }
    ...
    override async connect(): Promise<ListenerStatus> {
        ...
        if (this.server) return { ok: true, connected: true, message: 'callback server already running' }
    ...
    override async disconnect(): Promise<ListenerStatus> {
        ...
        if (!server) return { ok: true, connected: false, message: '本来就没有回调服务' }
    ...
    private async cryptoOf(): Promise<WeComCryptoLike> {
        if (this.cfg.cryptoModule) return this.cfg.cryptoModule
        try {
            const mod = (await import('@wecom/crypto')) as WeComCryptoLike
            if (mod.decrypt || mod.getSignature) return mod
    ...
    private async handleCallback(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
        ...
        try {
            ...
            log('error', 'wecom callback error:', (e as Error).message)
            res.statusCode = 500
            res.end('')
        }
    }

    private verifySignature(cryptoMod: WeComCryptoLike, sig: string, ts: string, nonce: string, encrypt: string): boolean {
    ...
    private decryptPayload(cryptoMod: WeComCryptoLike, encrypt: string): string {
     ...
     */
    async feed(payload: string | Record<string, string>): Promise<InboundMessage | null> {
        ...
        const chatType: 'single' | 'group' = xml.ChatType === 'group' ? 'group' : 'single'
        if (!chatId) {
            log('warn', 'wecom event without ChatId; dropped:', msgType, event)
            return null
        }
        if (chatType === 'group') this.knownChatTypes.set(chatId, 'group')
        else this.knownChatTypes.set(chatId, 'single')
        ...
        if (msgType === 'text') {
            ...
            if (local) imagePaths.push(local)
            else if (mediaId) imageErrors.push(`${mediaId.slice(-8)}: 图片下载失败`)
            text = '[图片]'
            ...
            if (!isUserAction) {
                ...
                if (!this.cfg.emitUnknownEvents) return null
            }
            text = `[事件: ${event}]`
        } else {
    ...
    private async tryDownloadImage(mediaId: string, picUrl: string): Promise<string | null> {
        ...
        const f = this.cfg.fetchImpl || fetch
        const attempt = async (url: string): Promise<Buffer> => {
        ...
        try {
            ...
            log('warn', 'wecom image PicUrl download failed, trying media api:', (e as Error).message)
        }
        try {
            const token = await this.getAccessToken()
            ...
            log('warn', 'wecom image media download failed:', (e as Error).message)
            return null
        }
    }
...
/** Convenience factory. */
export function createWeComChannel(cfg: WeComChannelConfig = {}): WeComChannel {
